...
const Search: React.FC = () => {
  ...
  useEffect(() => {
    if (
      searched &&
      (selectedGenres.length > 0 || selectedPlatforms.length > 0 || query)
    ) {
      handleFilterChange();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div className="search-page">
      <div className="search-header">
        ...
        <p className="search-intro">
          Discover your next favorite game! Search by title and filter by genre,
          platform, and more.
        </p>
      </div>
              ...
              {filterOptions.platforms.slice(0, 8).map((platform) => (
                <label key={platform.id} className="filter-option">
                  <input
                    ...
                    onChange={() =>
                      handlePlatformToggle(platform.id.toString())
                    }
                  />
        ...
        <div className="active-filters">
          ...
          <div className="filter-tags">
            {selectedGenres.map((genreSlug) => {
              const genre = filterOptions.genres.find(
                (g) => g.slug === genreSlug
              );
              return genre ? (
            ...
            {selectedPlatforms.map((platformId) => {
              const platform = filterOptions.platforms.find(
